...
const EuroreparSatimGoster = () => {
  ...
  const handleDelete = async (itemId) => {
    try {
      ...
      if (userConfirmed) {
        await axios.delete(
          `${process.env.REACT_APP_SERVER_URL}/api/eurorepar/kayit/${itemId}`
        );
        fetchData();
        fetchTotalSales();
      } else {
  ...
  const handleSaveEdit = async () => {
    try {
      const response = await axios.put(
        `${process.env.REACT_APP_SERVER_URL}/api/eurorepar/kayit/${editItemId}`,
        editedData
      );
  ...
  return (
    <Container className="mt-5">
      ...
      <Table striped bordered hover>
        ...
        <tbody>
          {EuroreparsAkuData.slice().reverse().map((item) => (
            <tr key={item._id}>
...
export default EuroreparSatimGoster;
